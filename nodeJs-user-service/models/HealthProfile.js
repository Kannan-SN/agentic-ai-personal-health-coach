            ...
            value: {
                ...
                validate: {
                    validator: function (value) {
                        const unit = this.height.unit
                        if (unit === 'inches') {
                            return value >= 30 && value <= 120 
                        } else if (unit === 'cm') {
                            return value >= 76 && value <= 305 
                        }
                        return false
                    },
                    message: 'Height must be within reasonable range for the selected unit',
                },
            },
            unit: {
                type: String,
...
HealthProfileSchema.methods.requiresProfessionalConsultation = function () {
    ...
    if (this.riskFactors.some((factor) => factor.severity === 'high' || factor.severity === 'critical')) return true

    if (this.medications.some((med) => med.affectsExercise || med.affectsNutrition)) return true

    if (this.injuries.some((injury) => injury.status === 'healing' || injury.status === 'requires_therapy')) return true
